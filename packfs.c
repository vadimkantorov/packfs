...
void packfs_dir_add_dirname(const char* path)
{
    ...
    for(const char* res = strchr(path, packfs_sep), *prevres = path; prevres != NULL; prevres = res, res = (res != NULL ? strchr(res + 1, packfs_sep) : NULL))
    {
        ...
        {
            const char* entryabspath = packfs_dynamic_dirpaths + offset;
            if(0 == strncmp(path, entryabspath, dirname_len) && entryabspath[dirname_len] == '\0')
            {
                exists = 1;
                break;
            }
        ...
        if(!exists)
        {
            packfs_dir_add(path, dirname_len, "", 0);
        }
    }
}
    ...
    do
    {
        ...
        while(1)
        {
            ...
            if(entryisdir && !packfs_dir_exists(prefix, entrypath)) // TODO: execute after entrypath has trailing slash
            {
                ...
                packfs_dir_add_dirname(full_path);
    ...
    if(packfs_initialized == 1 && packfs_enabled == 0)
    {
        ...
        if(packfs_config != NULL && packfs_config[0] != '\0')
        {
            ...
            for(const char* begin = packfs_config, *end = strchr(packfs_config, packfs_pathsep), *prevend  = packfs_config; prevend != NULL && begin[0] != '\0'; )
                ...
                prevend = end, begin = (end + 1), end = (end != NULL ? strchr(end + 1, packfs_pathsep) : NULL);
            {
                        ...
                        for(struct dirent* entry = __real_readdir(dirptr); entry != NULL; entry = __real_readdir(dirptr))
                        {
                            ...
                            if(path_prefix_len > 0)
                            {
                                ...
                                if(fileptr != NULL)
                                {
                                    ...
                                    packfs_scan_archive(fileptr, a, _path_normalized, prefix);
