...
#define PACKFS_ARCHIVE_READ_SUPPORT_FORMAT(a) {}
#endif
#endif
...
#ifdef PACKFS_ARCHIVE
PACKFS_STRING_VALUE(PACKFS_ARCHIVE)
#else
""
#endif
;
...
enum 
{
    ...
    PACKFS_ERROR_BAD = -1, // PACKFS_EOF = EOF,
    PACKFS_ERROR = 1, // PACKFS_ERROR = ARCHIVE_EOF,
    PACKFS_OK = 0, // PACKFS_OK = ARCHIVE_OK
};
...
enum
{
    ...
    packfs_filesdirs_ino_range = 50000000,

    packfs_static_files_ino_begin  = 1000000000,
    packfs_static_files_ino_end    = packfs_static_files_ino_begin  + packfs_filesdirs_ino_range,
    packfs_static_dirs_ino_begin   = packfs_static_files_ino_end,
    packfs_static_dirs_ino_end     = packfs_static_dirs_ino_begin   + packfs_filesdirs_ino_range,

    packfs_dynamic_files_ino_begin = 2000000000,
    packfs_dynamic_files_ino_end   = packfs_dynamic_files_ino_begin + packfs_filesdirs_ino_range,
    packfs_dynamic_dirs_ino_begin  = packfs_dynamic_files_ino_end,
    packfs_dynamic_dirs_ino_end    = packfs_dynamic_dirs_ino_begin  + packfs_filesdirs_ino_range,
};
...
static void packfs_sep_strncat(char* str, const char sep, const size_t str_sizeof, size_t* str_len, const char* part, const size_t part_len)
{ 
    if(str_len != NULL)
    {
        bool need_prepend = str_len > 0 && sep != '\0';
        if((*str_len) + 1 * need_prepend + part_len < str_sizeof)
        {
            if(need_prepend)
            { 
                str[(*str_len)++] = sep;
            ...
            if(part_len > 0)
            {
                strncpy(str + (*str_len), part, part_len); 
                (*str_len) += part_len; 
            }
            str[*str_len] = '\0';
        }
    ...
    {
        if(part_len < str_sizeof)
        {
            if(part_len > 0)
            {
                strncpy(str, part, part_len);
            }
            str[part_len] = '\0';
        }
    }
...
bool packfs_fd_in_range(const int fd)
{
...
static bool packfs_normpath(const char* path, char* path_normalized, const size_t path_normalized_sizeof)
{
    ...
    for(size_t i = (path != NULL && len > 2 && path[0] == packfs_extsep && path[1] == packfs_sep) ? 2 : 0; len > 0 && i < len; i++)
    {
...
static bool packfs_resolve_relative_path(char* dest, const size_t dest_sizeof, const int dirfd, const char* path)
{
    ...
    PACKFS_SPLIT_FOR(packfs_static_dirs_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
    {
...
bool packfs_match_ext(const char* path, const size_t path_len, const char* exts)
{
    ...
    PACKFS_SPLIT_FOR(exts, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
    {
...
static size_t packfs_calc_archive_prefixlen(const char* path, const char* exts)
{
    ...
    PACKFS_SPLIT_FOR(path, packfs_sep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
    {
...
bool packfs_match_path(const char* haystack, const size_t haystack_len, const char* needle, const size_t needle_len, const enum packfs_match_path_mode mode)
{
    ...
    switch(mode)
    {
        ...
        {
            const size_t needle_len_m1 = (needle[needle_len - 1] == packfs_sep) ? (needle_len - 1) : needle_len;
            const size_t needle_len_m1p1 = needle_len_m1 + 1;

            PACKFS_SPLIT_FOR(haystack, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
            {
                if(0 != strncmp(entryabspath, needle, needle_len_m1))
                    continue;

                if(((entryabspath_len == needle_len_m1) || (entryabspath_len == needle_len_m1p1 && entryabspath[needle_len_m1] == packfs_sep && (entryabspath[needle_len_m1p1] == '\0' || entryabspath[needle_len_m1p1] == packfs_pathsep))))
                    return true;
            }
            return false;
        }
...
bool packfs_dynamic_add_file(const char* prefix, const size_t prefix_len, const char* entrypath, const size_t entrypath_len, const size_t size, const size_t offset, const size_t archivepaths_offset)
{
    ...
    packfs_dynamic_files_paths_prefixlen[packfs_dynamic_files_num] = prefix_len_m1p1;
...
bool packfs_dynamic_add_dirname(const char* prefix, const size_t prefix_len, const char* entrypath, const size_t entrypath_len, const bool isdir)
{
    ...
    if(isdir && path[path_len - 1] != packfs_sep)
        packfs_sep_strncat(path, packfs_sep, sizeof(path), &path_len, "", 0);

    const size_t prev_packfs_dynamic_dirs_num = packfs_dynamic_dirs_num, prev_packfs_dynamic_dirs_paths_len = packfs_dynamic_dirs_paths_len;

    PACKFS_SPLIT_FOR(path + prefix_len_m1 - 1, packfs_sep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len_m1p1, i, islast)
    {
        if(!islast)
        {
...
#if HAVE__FSEEKI64
    if (_fseeki64(mine->fileptr, seek, whence) == 0) {
        return _ftelli64(mine->fileptr);
    }
#elif HAVE_FSEEKO
    if (fseeko(mine->fileptr, seek, whence) == 0) {
        return ftello(mine->fileptr);
    }
#else
    if (fseek(mine->fileptr, seek, whence) == 0) {
        return ftell(mine->fileptr);
    }
#endif
...
static ssize_t packfs_archive_read_callback(struct archive *a, void *client_data, const void **buff)
{
    ...
    *buff = mine->buffer;
    size_t bytes_read = fread(mine->buffer, 1, sizeof(mine->buffer), mine->fileptr);
    if (bytes_read < sizeof(mine->buffer) && ferror(mine->fileptr)) {
            archive_set_error(a, errno, "Error reading file");
    }
    return (bytes_read);
}

bool packfs_dynamic_scan_archive(const char* packfs_archive_filename, const char* prefix)
{
    FILE* fileptr = __real_fopen(packfs_archive_filename, "r");
    ...
    PACKFS_ARCHIVE_READ_SUPPORT_FORMAT(a);

    struct archive_entry *entry;
    struct packfs_archive_data client_data;
    do
    {
        res = packfs_dynamic_add_dirname(prefix, prefix_len_m1, "", 0, true); if(!res) break;
        res = packfs_dynamic_add_prefix(prefix, prefix_len_m1); if(!res) break;
        ...
        int archive_res = archive_read_open1(a);
        
        // res = archive_read_open_FILE(a, f);
        if(archive_res != ARCHIVE_OK) { fprintf(stderr, "#%s\n", archive_error_string(a)); res = false; break; }
        
        for(;;)
        {
            archive_res = archive_read_next_header(a, &entry);
            if (archive_res == ARCHIVE_EOF)
            {
                res = true;
                break;
            }
            if (archive_res != ARCHIVE_OK)
            {
                res = false;
            ...
            archive_res = archive_read_data_block(a, &firstblock_buff, &firstblock_len, &firstblock_offset);
            ...
            archive_res = archive_read_data_skip(a);
            if (archive_res == ARCHIVE_EOF)
            {
                res = false;
                break;
    ...
    archive_read_free(a);
#endif
    if(0 != __real_fclose(fileptr)) return false;
    return res;
}

bool packfs_extract_archive_entry_from_FILE_to_FILE(FILE* f, const char* entrypath, const size_t entrypath_len, FILE* h)
{
#ifdef PACKFS_ARCHIVE
    struct archive *a = archive_read_new();
    PACKFS_ARCHIVE_READ_SUPPORT_FORMAT(a);
    ...
    do
    {
        ...
        for (;;)
        {
            ...
            if(match)
            {
                ...
                for (;(res = archive_read_data_block(a, &buff, &size, &offset)) == ARCHIVE_OK;)
                {
    ...
    archive_read_free(a);
#endif
    return true; //res == ARCHIVE_OK;
}

bool packfs_dynamic_scan_archive_dir(const char* path_normalized, const char* prefix)
{
    DIR* dirptr = __real_opendir(path_normalized);
    if(dirptr == NULL) return false;
    bool res = true;

    size_t path_normalized_len = packfs_path_len(path_normalized);
    ...
    for(struct dirent* entry = __real_readdir(dirptr); entry != NULL; entry = __real_readdir(dirptr))
    {
        ...
        if(archive_prefixlen > 0)
        {
            ...
            res = packfs_dynamic_scan_archive(_path_normalized, prefix) && res; // something below seems to trigger again packfs_init();
        }
    }
    if(0 != __real_closedir(dirptr)) return false;
    return res;
}

bool packfs_dynamic_scan_path(const char* input_path)
{
    struct stat path_stat;

    if(PACKFS_EMPTY(input_path)) return false;
    if(input_path == NULL || 0 != stat(input_path, &path_stat)) return false;
    ...
    if(isdir)
    {
        ...
        const int flags = 0;
        return PACKFS_OK == nftw(input_path, packfs_list_files_dirs, fd_limit, flags);
    }
    else if(isfile)
    {
        struct stat path_stat;
        ...
        return packfs_dynamic_add_file("", 0, input_path + basename_offset, basename_len, size, offset, archivepaths_offset) ? PACKFS_OK : PACKFS_ERROR;
    }

    return true;
}

bool packfs_dynamic_scan_listing(const char* packfs_listing_filename, const char* prefix, const char* prefix_archive)
{
    FILE* fileptr = __real_fopen(packfs_listing_filename, "rb");
    if(fileptr == NULL) return false;
    bool res = true;
    ...
    do
    {
        ...
        for(;;)
        {
            ...
            fscanf(fileptr, "{\n");
            const int ret_path = fscanf(fileptr, "\"path\"\n:\n\"%[^\"]\"", entrypath);
            if(ret_path != 1) { res = PACKFS_ERROR; break; }
            const size_t entrypath_len = strlen(entrypath);
            size_t size = 0, offset = 0;
            fscanf(fileptr, ",\n");
            const int ret_size = fscanf(fileptr, "\"size\"\n:\n%zu", &size);
            if(ret_size != 1) { res = PACKFS_ERROR; break; }
            fscanf(fileptr, ",\n");
            const int ret_offset = fscanf(fileptr, "\"offset\"\n:\n%zu", &offset);
            if(ret_offset != 1) { res = PACKFS_ERROR; break; }
            fscanf(fileptr, "}\n");
            fscanf(fileptr, ",\n");
    ...
    while(0);
    if(__real_fclose(fileptr) != 0) return false;
    return res;
}
...
bool packfs_init(const char* path, const char* packfs_config)
{
    ...
    if(packfs_initialized == true && packfs_enabled == false)
    {
        ...
        {
            PACKFS_SPLIT_FOR(packfs_config, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
            {
                char path_normalized[packfs_path_max] = {0}; 
                packfs_sep_strncat(path_normalized, packfs_atsep, sizeof(path_normalized), NULL, entryabspath, entryabspath_len);
                ...
                const bool isdir = path_normalized_len >= 1 && (path_normalized[path_normalized_len - 1] == packfs_sep);
                ...
                if(match)
                {
                    ...
                    if(fileptr != NULL)
                    {
                        ...
                        packfs_enabled = true;
                        if(!packfs_dynamic_scan_listing(path_normalized, prefix, prefix_archive))
                            res = false;
                    }
                }
                else if(isdir)
                {
                    ...
                    if(dirptr != NULL)
                    {
                        ...
                        packfs_enabled = true;
                        if(!packfs_dynamic_scan_archive_dir(path_normalized, prefix))
                            res = false;
                    }
                }
                else
                {
                    ...
                    if(fileptr != NULL)
                    {
                        ...
                        if(!packfs_dynamic_scan_archive(path_normalized, prefix))
                            res = false;
                    }
                }
            }
...
void* packfs_find(const int fd, void* ptr)
{
    if(ptr != NULL)
    {
        for(size_t k = 0; k < packfs_descr_fd_cnt; k++)
...
void* packfs_readdir(void* stream)
{
    ...
    { \
        ...
        const int has_trailing_slash = (entryabspath)[(entryabspath_len) - 1] == packfs_sep; \
        char path_normalized[packfs_path_max] = {0}; \
        packfs_sep_strncat(path_normalized, packfs_pathsep, sizeof(path_normalized), NULL, (entryabspath), (has_trailing_slash ? ((entryabspath_len) - 1) : (entryabspath_len))); \
        ...
        packfs_sep_strncat(dir_entry->d_name, packfs_pathsep, sizeof(dir_entry->d_name), NULL, (entryabspath) + basename_offset, basename_len); \
    }
    ...
    if(listing_static_files || listing_static_dirs)
    {
        ...
        if(listing_static_dirs)
        {
            PACKFS_SPLIT_FOR(packfs_static_dirs_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
            {
                ...
                const int i_match = d_ino < (packfs_static_dirs_ino_begin + i);
                if(i_match && match)
                {
        ...
        {
            const bool match = packfs_match_path(dirabspath, dirabspath_len, entryabspath, entryabspath_len, PACKFS_ENTRY_IN_DIR);
            const int i_match = (d_ino < (packfs_static_files_ino_begin + i)) || (i == 0 && listing_static_dirs);
            if(i_match && match)
            {
    ...
    else if(listing_dynamic_files || listing_dynamic_dirs)
    {
        ...
        if(listing_dynamic_dirs)
        {
            ...
            {
                const bool match = packfs_match_path(dirabspath, dirabspath_len, entryabspath, entryabspath_len, PACKFS_ENTRY_IN_DIR);
                const int i_match = d_ino < (packfs_dynamic_dirs_ino_begin + i);
                if(i_match && match)
                {
        ...
        {
            const bool match = packfs_match_path(dirabspath, dirabspath_len, entryabspath, entryabspath_len, PACKFS_ENTRY_IN_DIR);
            const int i_match = (d_ino < (packfs_dynamic_files_ino_begin + i)) || (i == 0 && listing_dynamic_dirs);
            if(i_match && match)
            {
...
int packfs_stat(const char* path, const int fd, bool* isdir, size_t* size, size_t* d_ino)
{
    if(PACKFS_EMPTY(path) && !packfs_fd_in_range(fd)) return PACKFS_ERROR_NOTINRANGE;
    ...
    if(packfs_path_in_range(packfs_static_prefix, path_normalized))
    {
        ...
        PACKFS_SPLIT_FOR(packfs_static_dirs_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
        {
            ...
            if(match)
            {
                *size = 0;
                *isdir = true;
                *d_ino = packfs_static_dirs_ino_begin + i;
                return PACKFS_OK;
            }
        }
        ...
        PACKFS_SPLIT_FOR(packfs_static_files_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
        {
            ...
            if(match)
            {
                ...
                *isdir = false;
                *d_ino = packfs_static_files_ino_begin + i;
                return PACKFS_OK;
            }
        }
    }
    ...
    if(packfs_path_in_range(packfs_dynamic_prefix, path_normalized))
    {
        ...
        PACKFS_SPLIT_FOR(packfs_dynamic_dirs_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
        {
            ...
            if(match)
            {
                *size = 0;
                *isdir = true;
                *d_ino = packfs_dynamic_dirs_ino_begin + i;
                return PACKFS_OK;
            }
        }
        ...
        PACKFS_SPLIT_FOR(packfs_dynamic_files_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
        {
            ...
            if(match)
            {
                ...
                *isdir = false;
                *d_ino = packfs_dynamic_files_ino_begin + i;
                return PACKFS_OK;
            }
        }
    }
...
void* packfs_open(const char* path, const int flags)
{
    if(PACKFS_EMPTY(path)) return NULL;
    ...
    if(packfs_path_in_range(packfs_dynamic_prefix, path_normalized))
    {
        ...
        PACKFS_SPLIT_FOR(packfs_dynamic_files_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast) 
        {
            ...
            char archivepath_[packfs_path_max] = {0};
            packfs_sep_strncat(archivepath_, packfs_pathsep, sizeof(archivepath_), NULL, archivepath, packfs_path_len(archivepath)); 
            if(match)
            {
                path_in_range = true;
                isdir = false;
...
bool packfs_cat_files_offsets(const char* output_path)
{
    ...
    PACKFS_SPLIT_FOR(packfs_dynamic_files_paths, packfs_pathsep, entryabspath, entryabspath_offset, entryabspath_len, prefix_len, i, islast)
    {
        ...
        packfs_sep_strncat(tmp_path, packfs_pathsep, sizeof(tmp_path), NULL, entryabspath, entryabspath_len);
...
int main(int argc, const char **argv)
{
    ...
    int res = PACKFS_OK;
    struct stat path_stat;
    if(input_path == NULL || 0 != stat(input_path, &path_stat)) { res = PACKFS_ERROR; fprintf(stderr, "Input path not specified or does not exist\n"); return res; }
    bool isfile = S_ISREG(path_stat.st_mode), isdir = S_ISDIR(path_stat.st_mode);
    if(!isfile && !isdir) { res = PACKFS_ERROR; fprintf(stderr, "Input path not file or dir\n"); return res; }
    ...
    if(listing)
    {
        ...
        if(!fileptr) { res = PACKFS_ERROR; fprintf(stderr, "#could not open input file: %s\n", input_path); return res; }
        res = packfs_dynamic_scan_archive(input_path, prefix) ? PACKFS_OK : PACKFS_ERROR; removeprefix = input_path;
        fclose(fileptr);

        packfs_dump_listing(output_path, removeprefix);

        if(object)
        {
            
            packfs_sep_strncat(tmp_path, '\0', sizeof(tmp_path), &tmp_path_len, output_path, strlen(output_path));
            packfs_sep_strncat(tmp_path, packfs_extsep, sizeof(tmp_path), &tmp_path_len, "h", 1);
    ...
    else if(object)
    {
        ...
        res = packfs_dynamic_scan_path(input_path) ? PACKFS_OK : PACKFS_ERROR; removeprefix = isdir ? input_path : "";
    ...
    if(object)
    {
        res = packfs_dump_static_package(header_path, removeprefix, package_path, prefix, ld);
        fprintf(stderr, "%s\n", header_path);
        fprintf(stderr, "%s.o\n", header_path);
