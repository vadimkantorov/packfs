...
//FILE* (*__real_fopen)(const char *path, const char *mode), int (*__real_fclose)(FILE* stream)
{
    ...
    do
    {
        ...
        strncpy(packfs_archive_entries_names + packfs_archive_entries_names_total, "/", 1);
...
int packfs_indir(const char* dirpath, const char* path)
{
    ...
    if(0 == strncmp(dirpath + first_slash, path, dirpath_len - first_slash) && trailing_slash == (path + dirpath_len - first_slash))
        return 1;
    return 0;
}
