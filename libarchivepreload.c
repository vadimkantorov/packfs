    ...
    if(packfs_ctx.packfs_initialized == 1 && packfs_ctx.packfs_enabled == 0)
    {
        ...
        else if(path != NULL)
        {
            ...
            if(path_prefix_len > 0)
            {
                ...
                packfs_ctx.packfs_enabled = 1;
                packfs_scan_archive(&packfs_ctx, path_sanitized, path_sanitized);
            }
        }
    }
...
FILE* packfs_open(struct packfs_context* packfs_ctx, const char* path)
{
    ...
    fprintf(stderr, "packfs_open1: '%s' '%s' %d %d\n", packfs_ctx->packfs_archive_prefix, path_sanitized, packfs_ctx->packfs_archive_entries_num, packfs_path_in_range(packfs_ctx->packfs_archive_prefix, path_sanitized));
